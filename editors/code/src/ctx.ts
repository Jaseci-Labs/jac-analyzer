...
import { isJacDocument } from "./util";
import type { JacAnalyzerExtensionApi } from "./main";
...
export function fetchWorkspace(): Workspace {
    ...
    const jacDocuments = vscode.workspace.textDocuments.filter((document) =>
        isJacDocument(document),
    );

    return folders.length === 0
        ? jacDocuments.length === 0
            ? { kind: "Empty" }
            : {
                    kind: "Detached Files",
                    files: jacDocuments,
...
export class Ctx implements JacAnalyzerExtensionApi {
